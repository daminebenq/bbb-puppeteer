...
timeout $TIMELIMIT_UPPER node dictate.js "$URL" "$basePath" $TIMELIMIT_SECONDS &> $basePath/dictate.out &
pids+=($!)
timeout $TIMELIMIT_UPPER node read.js "$URL" "$basePath" $TIMELIMIT_SECONDS &> $basePath/read.out &
pids+=($!)
